...
const DishCard = ({ dish, onAddToCart }) => (
  <div className="bg-phoenix-card rounded-xl shadow-md overflow-hidden transition hover:scale-105 hover:shadow-xl border border-phoenix-gold/20">
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <button onClick={() => onAddToCart(dish)} className="bg-phoenix-gold text-phoenix-dark hover:bg-phoenix-gold-light px-4 py-2 rounded-full text-sm font-bold transition">В корзину</button>
      </div>
    </div>
  </div>
);
...
export const Menu = () => {
  ...
  if (loading) return <div className="text-center py-12 text-phoenix-text">Загрузка меню...</div>;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {filteredItems.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-16">
            {filteredItems.map(dish => <DishCard key={dish.id} dish={dish} onAddToCart={addToCart} />)}
          </div>
        ) : (
