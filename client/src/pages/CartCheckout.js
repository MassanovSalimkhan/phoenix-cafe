        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7 space-y-6">
              ...
              <div className="space-y-6">
                {cart.map((item) => (
                  <div key={item.id} className="flex gap-4 sm:gap-6 items-center sm:items-start group">
                    <img src={item.image_url || "https://via.placeholder.com/80"} alt={item.name} className="w-20 h-20 sm:w-28 sm:h-28 rounded-2xl object-cover bg-stone-100" />
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="font-bold text-phoenix-text text-base sm:text-lg leading-tight truncate pr-2">{item.name}</h3>
                        <button onClick={() => removeFromCart(item.id)} className="text-phoenix-text-muted hover:text-red-500 transition"><Trash2 className="w-5 h-5" /></button>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center bg-phoenix-dark rounded-full border border-phoenix-gold/30 p-1">
                          <button onClick={() => updateCartQuantity(item.id, -1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-phoenix-card text-phoenix-gold hover:bg-phoenix-gold/20"><Minus className="w-4 h-4" /></button>
                          <span className="w-12 text-center font-bold text-phoenix-text text-sm">{item.quantity}</span>
                          <button onClick={() => updateCartQuantity(item.id, 1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-phoenix-card text-phoenix-gold hover:bg-phoenix-gold/20"><Plus className="w-4 h-4" /></button>
                        </div>
            ...
            <div className="bg-phoenix-card/50 rounded-3xl p-6 border border-phoenix-gold/20 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div><h3 className="font-bold text-phoenix-gold mb-1">Хотите забронировать столик?</h3><p className="text-phoenix-text-muted text-sm">Забронируйте столик заранее, чтобы мы подготовили всё к вашему приходу.</p></div>
              <Link to="/reservation" className="bg-phoenix-gold text-phoenix-dark hover:bg-phoenix-gold-light px-6 py-2.5 rounded-full font-bold text-sm transition whitespace-nowrap">Забронировать</Link>
            </div>
          </div>
          ...
          <div className="lg:col-span-5">
            <form onSubmit={handleCheckout} className="bg-phoenix-card rounded-3xl p-6 sm:p-8 shadow-md border border-phoenix-gold/20 sticky top-28">
              ...
              <div className="flex bg-phoenix-dark p-1 rounded-2xl mb-8">
                <button type="button" onClick={() => setOrderType("Delivery")} className={`flex-1 py-3 rounded-xl text-sm font-bold transition ${orderType === "Delivery" ? "bg-phoenix-gold text-phoenix-dark" : "text-phoenix-text-muted hover:text-phoenix-text"}`}>Доставка</button>
                <button type="button" onClick={() => setOrderType("Pickup")} className={`flex-1 py-3 rounded-xl text-sm font-bold transition ${orderType === "Pickup" ? "bg-phoenix-gold text-phoenix-dark" : "text-phoenix-text-muted hover:text-phoenix-text"}`}>Самовывоз</button>
              </div>
              {orderType === "Delivery" && (
              ...
              <div className="mb-8">
                ...
                <div className="space-y-3">
                  <label className="flex items-center p-4 border rounded-2xl border-phoenix-gold bg-phoenix-gold/10">
                    ...
                    <span className="ml-3 font-medium text-phoenix-text">Онлайн-оплата (имитация)</span>
                  </label>
                </div>
              </div>
